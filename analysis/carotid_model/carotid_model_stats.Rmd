...
#ggplot(survey_responses_data, aes(carotidconfidencepost, `Trial 2 TTH`, fill=carotidconfidencepost)) +
 # geom_boxplot()
```
...
overall_success_rate <- ggplot(success_rate_overall, aes(trial, rate)) +
  ...
  annotate('text', x = 2, y = 1.05, label=ifelse(fisher_success_overall$p.value < 0.05, '*', ''), size=12/ggplot2:::.pt) +
  # scale_fill_jama() +
  # facet_wrap(~Group) +
  xlab('Trial Number') +
...
by_group_success_rate <- ggplot(success_rate_by_group, aes(Group, rate, fill=trial)) +
  ...
  annotate('text', x = 1.25, y =1.05, label=ifelse(fisher_attendings_success_rate$p.value < 0.05, '*', ''), size=12/ggplot2:::.pt) +
  annotate('text', x = 2.25, y = 1.05, label=ifelse(fisher_residents_success_rate$p.value < 0.05, '*', ''), size=12/ggplot2:::.pt) +
  # facet_wrap(~Group) +
  xlab('Trial Number') +
  ylab('Trial success rate') +
  # facet_wrap(~Group) +
...
ggsave(figure1_bottom2, width = 10, height = 5, units = 'in', filename = file.path(plot_dir, 'overall_performance_alt_bottom.pdf'), device = 'pdf')

figure1_bottom_combined <- ggpubr::ggarrange(
  percent_change_in_scores_plot_on_single, labels=c('d')
)

ggsave(figure1_bottom_combined, width = 10, height = 5, units = 'in', filename = file.path(plot_dir, 'overall_performance_alt_combined_bottom.pdf'), device = 'pdf')
...
ggsave(suppfigure1, width = 10, height = 4, units = 'in', filename = paste0(plot_dir, '/overall_performance_by_group.pdf'), device = 'pdf')
...
ggsave(figure3, width = 10, height = 8, units = 'in', filename = paste0(plot_dir, '/trial_rescues.pdf'), device = 'pdf')
```
...
ggsave(figure4, width = 10, height = 5, units = 'in', filename = paste0(plot_dir, '/heart_rate.pdf'), device = 'pdf')
```
