...
Below we clean up the dataset (clean up column names, rerrange variables, etc) to ease manipulation later on for general analysis.
...
Below are some general stats of this dataset

```{r}
cleaned_dataset %>%
  filter(!is.na(SurveyID)) %>%
  distinct(SurveyID) %>% dim()

cleaned_dataset %>%
  filter(!is.na(SurveyID), !is.na(trial)) %>%
  distinct(SurveyID, trial) %>%
  dim()

cleaned_dataset %>%
  filter(!is.na(SurveyID), !is.na(tth)) %>%
  distinct(SurveyID, trial, tth) %>%
  dim()
```
...
Graphs: Heart-Rate graph showing tachycardia during the leak and then return to normal rate post 
