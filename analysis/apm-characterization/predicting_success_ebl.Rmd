...
ebl.train.data <- apms_fixed_c_names %>%
  mutate(EBL=log2(EBL)) %>%
  filter(trial_id %in% training_set) %>%
  # dplyr::select(c('EBL', 'total', 'Group', 'endolast12mo', 'cadaverlast12', 'resolution', columns_to_use[1:11])) %>%
...
ggsave(g6, filename = file.path(plot_dir, 'apms_1v30_fps.pdf'), units = 'in', height = 16, width = 12)
```
...
ggsave(g5, filename = file.path(plot_dir, 'tool_combinations_1v30_fps.pdf'), units = 'in', height = 10, width = 8)
```
...
ebl.train.data <- apms_30_fps_fixed_c_names %>%
  ...
  dplyr::select(intersect(colnames(.), c('EBL', 'total', 'Group', 'endolast12mo', 'cadaverlast12', 'resolution', columns_to_include)))
...
ggplot(data=rbind(ebl.train.data %>% mutate(s='train'), ebl.test.data %>% mutate(s='test')), aes(EBL, fill=s)) +
...
```{r}
g3 <- ggplot(predictions_ebl, aes(EBL, pred, color=model)) +
  geom_point(size=2) +
  geom_abline(slope=1, linetype=2) +
  xlab('Actual log2(EBL)') + ylab('Predicted log2(EBL)') + 
  stat_cor(method = "pearson", label.x = 6, label.y = 10) +
