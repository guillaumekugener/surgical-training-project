...
gt_fps1 <- read_csv('~/Documents/USC/USC_docs/ml/datasets/fps-1-uncropped-previous/ImageSets/Main/retinanet_surgical_1fps_complete.csv',
...
gt_fps1_test_only <- read_csv('~/Documents/USC/USC_docs/ml/datasets/fps-1-uncropped-previous/ImageSets/Main/retinanet_surgical_1fps_test.csv',
  col_names = c('file', 'x1', 'y1', 'x2', 'y2', 'tool')) %>%
  mutate(short_file_name=gsub('.*/', '', file)) %>%
  mutate(trial_id=gsub('[a-z]?_.*', '', short_file_name)) %>%
  mutate(frame=as.numeric(stringr::str_extract(pattern='[0-9]+', string=gsub('.*_', '', short_file_name))))
...
total_frames_by_trial <- gt_fps1 %>%
  group_by(trial_id) %>%
  dplyr::summarise(total=length(unique(short_file_name))*30)
```
...
retinanet_1_fps_results <- read_csv(file.path(save_location_results, 'test_retinanet_pred_retina_style.csv')) %>%
...
```{r include=FALSE}
# Load the detections
retinanet_30_fps_dir <- '~/Documents/USC/USC_docs/ml/datasets/fps-30-detection/'
